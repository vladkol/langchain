...
>
> BigQuery ML function [ML.DISTANCE](https://cloud.google.com/bigquery/docs/reference/standard-sql/bigqueryml-syntax-distance)
> computes the distance between two vectors.
> It can calculate Euclidean, Manhattan or Cosine distance. With LangChain, we use Cosine distance.
>
> `ML.DISTANCE` operates straight over your vectors stored in BigQuery, making vector search more efficient,
> especially with large sets of documents.

BigQueryVectorSearch also supports document retrieval by metadata,
with `metadata` field stored as [JSON](https://cloud.google.com/bigquery/docs/json-data) data type.
...
See a [usage example](/docs/integrations/vectorstores/bigquery).
